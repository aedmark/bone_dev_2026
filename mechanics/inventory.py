...
class GordonKnot:
    ...
    def enforce_object_action_coupling(self, user_input: str, current_zone: str) -> Optional[str]:
        ...
        # 2. Inventory Coupling Check (e.g., "chop" requires "axe")
    ...
    def load_config(self):
        ...
        data = LoreManifest.get_instance().get("GORDON") or (LoreManifest.get_raw("gordon.json") if hasattr(LoreManifest, "get_raw") else {})
    ...
    def process_loot_tags(self, text: str, user_input: str) -> Tuple[str, List[str]]:
        ...
        # If no explicit tags exist, try to parse implicit acquisition from dialogue
        if not new_loot:
        ...
        if new_loot:
            ...
            # If the LLM spawned it but the user didn't ask to take it, wait for consent.
            else:
                if self.events:
                    for item in new_loot:
        ...
        # Strip the tags from the final output payload
    ...
    def consume(self, item_name: str) -> Tuple[bool, str]:
        ...
        if item.function == "STABILITY":
            return True, f"{Prisma.MAG}{(ux('gordon_strings', 'consume_pizza') or '').format(item=item_name)}{Prisma.RST}"
