    ...
    def load_vocabulary(self):
        ...
        for cat, words in data.items():
            if cat not in ["solvents", "antigen_replacements"]:
                ...
                # Sentiment tracking relies on different mechanics, skip indexing them here.
...
class LinguisticAnalyzer:
    ...
    def measure_viscosity(self, word: str) -> float:
        ...
        if clean_word in self.store.SOLVENTS:
            return 0.1 # Solvents slip through frictionless.
    ...
    def vectorize(self, text: str) -> Dict[str, float]:
        ...
        # Entropy is calculated as a composite of Chaos (CHI) and vocabulary uniqueness.
    ...
    def measure_valence(self, words: List[str]) -> float:
        ...
        for word in words:
            ...
            if val != 0.0 and prev_negator:
                val *= -0.5 # Flip and dampen the sentiment if preceded by "not" or "never"
            score += val
...
class SemanticField:
    ...
    def update(self, text: str) -> Dict[str, float]:
        ...
        if not self.current_vector:
            ...
            self.current_vector = {
                dimension: round((self.current_vector.get(dimension, 0.0) * 0.6) + (new_vector.get(dimension, 0.0) * 0.4), 3)
