...
class TrueEngineTest(unittest.TestCase):
    ...
    def test_prompt_composer_anti_bleed_membranes(self):
        ...
        tech_prompt = composer.compose(
            tech_state, "Refactor this.", modifiers={"include_inventory": False}
        )
        self.assertIn(
            "Clinical, precise", tech_prompt, "TECHNICAL style guide missing."
        )
        self.assertIn(
            "Do not write prose, poetry, or narrative descriptions.",
            tech_prompt,
    ...
    def test_paradox_rest_and_orthogonal_attention(self):
        ...
        self.assertIn(
            "two mutually exclusive perspectives simultaneously",
            ortho_prompt,
        ...
        self.assertIn(
            "mathematically optimal to be unsure",
            paradox_prompt,
    ...
    def test_subconscious_matrix_absorption(self):
        ...
        try:
            ...
            self.assertEqual(
                sum(initial_vibe),
                0.0,
                "Initial matrix should yield a completely zeroed vibe.",
            )
            ...
            self.assertNotEqual(
                vibe_sum,
                0.0,
                "Matrix failed to absorb the K*V weights of the buried word.",
            )
            self.assertEqual(
                len(new_vibe), 8, "Vibe vector must be exactly 8-dimensional."
            )
        finally:
    ...
    def test_ghost_physics_haunting(self):
        ...
        try:
            network.subconscious.matrix_filepath = os.path.join(
                os.path.dirname(tmp_path), "test_m_t2.json"
            )
            network.subconscious.bury({"word": "echo", "mass": 10.0})
            physics = {
                "clean_words": ["echo", "hello"],
                "voltage": 10.0,
                "narrative_drag": 1.0,
            }
            ...
            self.assertNotEqual(
                physics["voltage"],
                10.0,
                "The ghost failed to mutate the system Voltage.",
            )
            self.assertNotEqual(
            ...
            self.assertIn(
                "ECHO", log, "The log string did not identify the haunting word."
            )
        finally:
            if os.path.exists(tmp_path):
                os.remove(tmp_path)
            if hasattr(network, "subconscious") and os.path.exists(
                network.subconscious.matrix_filepath
            ):
    ...
    def test_paradox_engine_ignition(self):
        ...
        self.assertIn(
            "non-negotiable truths", prompt, "Paradox prompt string is malformed."
        )
    ...
    def test_foothills_veil_hush(self):
        ...
        self.assertNotIn(
            "[BIO]", joined_logs, "CycleReporter leaked BIO tags in WARM mode."
        )
        self.assertNotIn(
            "[CRITIC]", joined_logs, "CycleReporter leaked CRITIC tags in WARM mode."
        )
        self.assertIn(
            "forest path",
            joined_logs,
    ...
    def test_retroactive_metabolism_and_sleep_isolated(self):
        ...
        try:
            ...
            ctx.limits = (
                getattr(self.engine.bone_config, "CYCLE", {}).__dict__
                if hasattr(getattr(self.engine.bone_config, "CYCLE", None), "__dict__")
                else {}
            )
            ctx = phase.run(ctx)
            self.assertEqual(
