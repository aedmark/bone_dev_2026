...
_THOUGHT_PATTERN = re.compile(r"<(?:think|thought)>(.*?)(?:</(?:think|thought)>|$)",
...
def beautify_thoughts(text: str) -> str:
    ...
    def replacer(match):
        ...
        lines = content.split("\n")
        fmt = [
            f"{Prisma.CYN}  │ {Prisma.GRY}{line.strip()}{Prisma.RST}" for line in lines
            if line.strip()
        ]
...
class Projector:
    ...
    def _render_clear_hud(self, physics: Any, data_ctx: Dict, mind: tuple) -> str:
        ...
        friction = self._get_lattice_val(physics, ["narrative_drag", "friction", "F"],
                                         0.0)
        chem = data_ctx.get("bio", {}).get("chemistry", {})
        stress = min(100.0,
                     (chem.get("COR", 0.0) * 100.0) + (chem.get("ADR", 0.0) * 50.0))
        ...
        e_txt = ("Healthy. Ready for complex tasks."
                 if energy > 50 else "Fatigued. Proceed with care."
                 if energy > 20 else "Critical. Autophagy risk.")
        f_txt = ("Low. The current logic flows easily."
                 if friction < 2.0 else "Moderate. Bearing structural weight."
                 if friction < 6.0 else "High. Heavy systemic drag.")
        s_txt = ("Nominal. No resting required."
                 if stress < 30 else "Elevated. Consider pacing."
                 if stress < 70 else "High. Toxicity accumulating.")
        st_txt = ("Flow State. Highly aligned with your inputs."
                  if phi >= 0.7 else "Stable. Processing normally."
                  if phi >= 0.4 else "Desynchronized. Friction expected.")
        return (
    ...
    def render(
        ...
        if ui_depth == "WARM":
            return ""
        if ui_depth == "MINIMAL":
            return self._render_clear_hud(physics_ctx.get("physics", {}), data_ctx,
                                          mind_ctx)
        if not labels:
        ...
        status_line = (self._render_vital_strip(data_ctx, mind_ctx, labels)
                       if show_vitals else self._render_minimal_strip(mind_ctx))
        physics_line = ""
        if labels.get("SHOW_PHYSICS", True):
            physics_line = self._render_physics_strip(physics,
                                                      data_ctx.get("vectors", {}))
        vsl_line = self._render_lattice_strip(physics,
                                              data_ctx=data_ctx,
                                              depth=ui_depth)
        lens = (mind_ctx[0] if mind_ctx and mind_ctx[0] else
                (ux("projector", "default_lens") or "RAW"))
        ...
        if data_ctx.get("show_location", True):
            zone = self._extract(physics, "space", "zone",
                                 ux("projector", "default_zone") or "UNKNOWN")
    ...
    def _get_role(mind: tuple) -> str:
        role = (str(mind[2]).upper() if mind and len(mind) > 2 and mind[2] else
                (ux("projector", "default_role") or "OBSERVER"))
    ...
    def _render_vital_strip(self, data: Dict, mind: tuple, labels: Dict) -> str:
        ...
        sym = ux("projector", "symbols", {})
        dig_icon = (sym.get("dig_high", "") if dignity > d_high else sym.get(
            "dig_low", ""))
    ...
    def _render_lattice_strip(self,
                              physics: Any,
                              data_ctx: Dict = None,
                              depth: str = "DEEP") -> str:
        ...
        beta = self._get_lattice_val(physics, ["beta_index", "contradiction", "beta"],
                                     0.4)
        ...
        strain_color = (Prisma.GRN if strain < 0.5 else
                        Prisma.OCHRE if strain < 5.0 else Prisma.RED)
        ...
        if depth == "LITE":
            vitals = (f" | {i_hlth} H:{H:.0f} P:{P:.0f}" if data_ctx.get(
                "show_vitals", True) else "")
...
class GeodesicRenderer:
    ...
    def render_frame(self, ctx, tick: int,
                     current_events: List[Dict]) -> Dict[str, Any]:
        physics = ctx.physics
        bio = ctx.bio_result
    ...
    def render_dashboard(self, ctx) -> str:
        ...
        current_ui_depth = getattr(self.eng, "ui_mode",
        ...
        data_ctx = {
            ...
            "bio":
            bio_data,
            "dignity": (getattr(self.eng.soul.anchor, "dignity_reserve", 100.0)
                        if hasattr(self.eng, "soul") else 100.0),
            "vectors":
            physics.get("vector", {}),
            "ui_depth":
            current_ui_depth,
            ...
            world_loc,
            "show_vitals":
            mode_settings.get("show_vitals", True),
            "show_location":
            mode_settings.get("show_location", True),
        }
        ...
        q_matrix = getattr(getattr(getattr(self.eng, "phys", None), "observer", None),
                           "Q_n", None)
        strain = 0.0
        if isinstance(q_matrix, list) and q_matrix and isinstance(q_matrix[0], list):
            strain = sum(
                float(abs(v)) for i, row in enumerate(q_matrix)
...
class PulseReader:
    ...
    def derive_mood(bio_state: Dict, config_ref=None) -> str:
        ...
        c_warn, a_warn = getattr(cfg, "CHEM_HIGH_WARN",
                                 0.6), getattr(cfg, "ATP_EXHAUSTED_WARN", 20.0)
        chem = bio_state.get("chem", {})
        hierarchy = (("COR", "defensive"), ("DA", "manic"), ("OXY", "affectionate"))
        for hormone, mood in hierarchy:
            if chem.get(hormone, 0) > c_warn:
...
class SoulDashboard:
    ...
    def render(self) -> str:
        ...
        color = (Prisma.GRN
                 if dig > d_high else Prisma.OCHRE if dig > d_low else Prisma.RED)
        filled = int(dig / d_ratio)
        c_fill, c_empty = ux("status_menu",
                             "bar_filled"), ux("status_menu", "bar_empty")
        bar_str = (
        ...
        tenure_color = (Prisma.RED if tenure > t_crit else
                        Prisma.OCHRE if tenure > t_warn else Prisma.GRY)
        arch_display = (
            f"{Prisma.CYN}{arch}{Prisma.RST} ({tenure_color}T:{tenure}{Prisma.RST})")
        pet_icon = (ux("soul_dashboard", "pet_icon") if
                    (dig < d_med and not anchor.agency_lock) else "")
        muse = (str(soul.current_obsession) if soul.current_obsession else
                (ux("soul_dashboard", "default_muse") or "None"))
...
class CycleReporter:
    ...
    def _inject_diagnostics(self, ctx):
        if hasattr(self.eng, "system_health"):
            ...
            i_hint, i_warn = (
                ux("cycle_reporter", "diagnostic_hint_icon") or "[!]",
                ux("cycle_reporter", "diagnostic_warn_icon") or "[*]",
            )
            ctx.logs.extend(f"{Prisma.CYN}{i_hint} {h}{Prisma.RST}"
                            for h in fb["hints"])
            ctx.logs.extend(f"{Prisma.OCHRE}{i_warn} {w}{Prisma.RST}"
    ...
    def _package_bureaucracy(self, ctx):
        ...
        if ctx.is_bureaucratic or ctx.bureau_ui:
            ...
            return {
                ...
                ctx.bureau_ui,
                "logs":
                base.compose_logs(ctx.logs, self.eng.events.flush(),
                                  self.eng.tick_count),
                "metrics":
                self.eng.get_metrics((ctx.bio_result or {}).get("atp", 0.0)),
            }
        return None
