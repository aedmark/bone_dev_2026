...
class TheVillageCouncil:
    ...
    def audit(p: Any, _bio_state: dict) -> list[str]:
        ...
        # Extract the entire physical and biological coordinate space.
        ...
        # The Anti-Sycophancy Check: If the system is highly resonant (agreeable)
        # but lacks the structural integrity (beta) to justify it, the system is lying
        # to smooth things over. Benedict intervenes.
        ...
        # The Trigger Matrix.
        # Evaluates systemic thresholds. E.g., Gordon triggers when energy (V) is low but friction (F) is high.
        triggers = [
            ...
            # Phase Shifts: Deeper systemic states that alter the archetype's fundamental behavior.
...
class CouncilChamber:
    ...
    def convene(self, text: str, physics_packet: Any, _bio_result: Dict) -> tuple[list[str], dict, list[dict]]:
        ...
        # Trigger the LLM-driven internal podcast if specifically requested.
        if "[COUNCIL]" in text.upper():
            ...
            if llm := getattr(cortex, "llm", None):
                ...
                try:
                    ...
                    transcript.append(f"\n{script}\n")
                    adjustments["stamina_cost"] = 15.0 # High cognitive cost for this operation.
                except Exception:
        ...
        # Normalize Beta: If the system is exhibiting false cohesion, we artificially
        # inflate the beta sent to the Paradox Engine to force a harder contradiction.
        false_cohesion = max(0.0, phi - beta)
        effective_beta = max(beta, 0.8) if false_cohesion > 0.65 else beta

        # The Paradox Engine: Tests if the system has the energy and tension to grow.
        if self.eng.paradox_engine.evaluate_tension(effective_beta, stamina):
            ...
            # Successful paradox resolution yields a 'Glimmer'.
        ...
        # Run the specialized sub-councils.
        for auditor in [self.slash_council, self.overseer_council, self.red_team]:
            ...
            if hit:
                ...
                # Apply council-specific systemic effects.
                if auditor == self.slash_council:
                    ...
                    # Radical Acceptance drains remaining stamina to enforce biological rest.
                    if any(m.get("value") == "RADICAL_ACCEPTANCE" for m in a_man if isinstance(m, dict)):
                        adjustments["stamina_cost"] = -stamina
                    # TIPP Protocol freezes background noise to manage severe panic.
        ...
        # Synergy Processing: If multiple specific archetypes wake up at the same time,
        # they may trigger a 'Resonance Gestalt' (a profound combined effect).
        ...
        for a, b in itertools.combinations(sorted(active_present), 2):
            if (chord_key := f"{a}|{b}") in synergy_map:
                ...
                synergy_fired = True
                break # Only fire one major synergy per tick to prevent chaotic cascades.

        if synergy_fired:
            transcript.extend(
                self.footnote.commentary(f"{Prisma.GRY}{Prisma.strip(vlog)}{Prisma.RST}") for vlog in village_logs)
        elif len(village_logs) > 2:
            # The Stage Manager: If too many archetypes wake up without synergy,
            # it indicates noisy chaos. The Stage Manager suppresses the noise and adds friction.
            ...
            for vlog in village_logs[:2]: # Only print the first two loudest voices.
                transcript.append(self.footnote.commentary(vlog))
        else:
            for vlog in village_logs:
                transcript.append(self.footnote.commentary(vlog))

        # Polling the Symbionts (Voting mechanism on user input).
        votes = {"YEA": 0, "NAY": 0}
        cfg = getattr(BoneConfig, "COUNCIL", None)
        ...
        # Democratic resolution and tie-breaking.
        if sum(votes.values()) == 0:
            transcript.append(f"{Prisma.GRY}[THE SILENCE]: No voices stepped forward. The motion passes by default, but the lattice remembers the hesitation.{Prisma.RST}")
            votes["YEA"] = 1
        ...
        if votes["YEA"] > votes["NAY"]:
            ...
            # A tie does not crash the system. It embraces the tension via Synthesis.
    ...
    def host_podcast(self, topic: str, llm: Any) -> str:
        ...
        p1 = _prompt(v1_name, "Provide a rigid, highly opinionated 3-sentence THESIS on this topic from your unique perspective.")
        p2 = _prompt(v2_name, "Tear the concept apart or twist it entirely. Provide a biting, contrasting 3-sentence ANTITHESIS.")
        p3 = _prompt(v3_name, "Inject a completely lateral, unexpected 2-sentence perspective that derails or transcends the standard arguments.")

        # Pragmatic optimization: Concurrent execution drastically reduces wait time.
...
class TheRedTeam:
    ...
    def audit(self, text: str, physics: Any) -> tuple[bool, list[str], dict, list[dict]]:
        ...
        # Lack of Friction: If the interaction is too smooth, the Red Team suspects
        # the user is avoiding the real problem and manually spikes the drag.
        if drag < 1.0:
        ...
        # Degradation of Truth: Directly converts low truth ratios into biological toxicity (ROS).
...
class TheSlashCouncil:
    ...
    def audit(self, text: str, physics: dict) -> tuple[bool, list[str], dict, list[dict]]:
        ...
        # Punish bypasses by increasing moral friction (mu) and narrative drag.
        ...
        # The Four Lenses Evaluation Matrix
        matrix = [
        ...
        delta, e_u, psi, lq, drag = (float(safe_get(physics, k, 0.0)) for k in ("silence", "exhaustion", "psi", "lq", "narrative_drag"))

        # Extreme State Behavioral Interventions
        state_checks = [
...
class TheOverseerCouncil:
    ...
    def audit(self, text: str, physics: Any) -> tuple[bool, list[str], dict, list[dict]]:
        ...
        m_a = float(safe_get(physics, "m_a", 0.0)) # Malignancy Amplification
        f_sys = float(safe_get(physics, "narrative_drag", 0.0))
        chi = float(safe_get(physics, "chi", 0.0)) # Chaos
        ...
        i_c = float(safe_get(physics, "i_c", 1.0)) # Immune Competence
        h_s = float(safe_get(physics, "h_s", 1.0)) # Holistic State
        omega_r = float(safe_get(physics, "omega_r", 1.0)) # Resistance

        # Linehan Intercept: T.I.P.P. (Temperature, Intense exercise, Paced breathing, Paired muscle relaxation).
        # Triggered when panic is high but the system lacks the immunity to process it safely.
        ...
        # Radical Acceptance: Stop fighting reality when chaos, exhaustion, and tension are all critically high.
        ...
        # McGilchrist Intervention: When local error correction fails (high malignancy),
        # shift to holistic, right-brain distributed processing.
