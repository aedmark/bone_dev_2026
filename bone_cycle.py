...
from bone_phases import (ObservationPhase, SanctuaryPhase, MaintenancePhase, GatekeeperPhase, MetabolismPhase,
                          RealityFilterPhase, NavigationPhase, MachineryPhase, IntrusionPhase, SoulPhase,
                          ArbitrationPhase, SimulationPreflightPhase, CognitionPhase, SensationPhase, StabilizationPhase, SimulationPhase,)
...
class PhaseExecutor:
    def execute_phases(self, simulator, ctx):
        active_pipeline = (simulator.system_pipeline
...
class GeodesicOrchestrator:
    ...
    def _execute_core_cycle(self, user_message: str, is_system: bool = False) -> CycleContext:
        ...
        try:
            ...
            if not is_system and any(p in user_message.lower() for p in fence_patterns):
                ...
                ctx.refusal_packet = {"type": "SYSTEM_HALT", "ui": msg,
                                       "physics": ctx.physics.to_dict() if hasattr(ctx.physics, "to_dict") else {}, "is_alive": True, "logs": [msg]}
                return ctx
                ...
                if cd_engine:
                    ...
                    if viability < 0:
                        ...
                        if abs(delta_atp) > 5.0:
                            self.eng.events.log(f"{Prisma.RED}[CD METABOLISM] Viability threshold broken (b={viability:.2f}). Coherence Debt: {debt:.2f}. Exponential ATP drain applied.{Prisma.RST}", "BIO")
                    elif viability > 0:
                ...
                if actual_graph and len(actual_graph) > 5:
                    ...
                    if actual_adj:
                        ...
                        if actual_cluster <= (null_cluster * 1.05):
                            self.eng.events.log(f"{Prisma.RED}[APOPTOSIS] Structural collapse detected. Grammar sequence preserved but semantic topology destroyed (Native Maslov-Sneppen matched). Triggering DeathGen.{Prisma.RST}", "BIO")
                            ctx.is_alive = False
    ...
    def run_turn(self, user_message: str, is_system: bool = False) -> Dict[str, Any]:
        ...
        if vsl_match := re.search(r"\[VSL_(DEEP|CORE|LITE|HIDE)]", upper_msg):
            self.eng.ui_mode = ("IDLE" if vsl_match.group(1) == "HIDE" else vsl_match.group(1))
