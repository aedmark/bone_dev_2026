...
class SanctuaryPhase(SimulationPhase):
    ...
    def _trigger_dream(self, ctx: CycleContext):
        ...
        soul_snapshot = (self.eng.soul.to_dict()
...
class MaintenancePhase(SimulationPhase):
    ...
    def run(self, ctx: CycleContext):
        if hasattr(self.eng, "town_hall"):
            ...
            session_snapshot = {"trauma_vector": self.eng.trauma_accum,
    ...
    def run(self, ctx: CycleContext):
        ...
        if self.eng.gordon:
            ...
            if coupling_error:
                ...
                ctx.refusal_triggered = True
                ctx.refusal_packet = {"type": "PREMISE_VIOLATION", "ui": f"\n{coupling_error}", "logs": [log_msg] if log_msg else [], "metrics": self.eng.get_metrics(), }
                return ctx
        ...
        if self.eng.bureau:
            ...
            if audit_result:
                if audit_result.get("block", False):
                    ...
                    ctx.refusal_packet = {"type": "BUREAU_BLOCK", "ui": ui_msg, "logs": [log_msg] if log_msg else [], "metrics": getattr(self.eng, "get_metrics", lambda: {})()}
                    return ctx
                if self.eng.bio and self.eng.bio.mito:
                    self.eng.bio.mito.adjust_atp(audit_result.get("atp_gain", 0.0), ux("cycle_strings", "gatekeep_bureau_fine"))
...
class MetabolismPhase(SimulationPhase):
    ...
    def run(self, ctx: CycleContext):
        ...
        metrics = self.eng.get_metrics()
        ctx.bio_result = self.eng.soma.digest_cycle(ctx.input_text, physics, bio_feedback, metrics["health"], metrics["stamina"], self.eng.bio.governor.get_stress_modifier(self.eng.tick_count), self.eng.tick_count, circadian_bias=self._check_circadian_rhythm(ctx), )
    ...
    def _apply_economic_stimulus(self, ctx: CycleContext, efficiency: float):
        import math
        base_cost = 0.0
        if efficiency < 0.8: base_cost = min(1.5, (0.8 - efficiency) * 5.0)
...
class RealityFilterPhase(SimulationPhase):
    ...
    def run(self, ctx: CycleContext):
        ...
        if vector and (sufficient_mass or sufficient_tension or "trigram" not in ctx.world_state):
...
class NavigationPhase(SimulationPhase):
    ...
    def run(self, ctx: CycleContext):
        ...
        if v_floor is not None:
            ...
            if v_floor >= 50.0:physics.narrative_drag = 0.0
...
class IntrusionPhase(SimulationPhase):
    ...
    def run(self, ctx: CycleContext):
        ...
        if (drag > ctx.limits.get("INTRUSION_DRAG_THRESH", 4.0) or kappa < ctx.limits.get("INTRUSION_KAPPA_THRESH", 0.3)) and ctx.clean_words:
        ...
        if (trauma_sum > ctx.limits.get("INTRUSION_NIGHTMARE_THRESH", 10.0) or is_bored) and random.random() < ctx.limits.get("INTRUSION_DREAM_CHANCE", 0.2):
...
class ArbitrationPhase(SimulationPhase):
    ...
    def run(self, ctx: CycleContext):
        ...
        synergy_name = None
        for mandate in getattr(ctx, "council_mandates", []):
            if mandate.get("action") == "SYNERGY_FIRED":
                synergy_name = mandate.get("value")
                break
...
class CognitionPhase(SimulationPhase):
    ...
    def run(self, ctx: CycleContext):
        ...
        if ctx.is_alive and ctx.clean_words:
            ...
            bury_msg, new_wells = self.eng.mind.mem.bury(ctx.clean_words, self.eng.tick_count, resonance=ctx.physics.voltage, desperation_level=desperation, learning_mod=learn_mod, )
            if bury_msg:
        ...
        ctx.mind_state = self.eng.noetic.think(physics_packet=ctx.physics.to_dict(), _bio=ctx.bio_result, _inventory=inventory_data, voltage_history=self.eng.phys.dynamics.voltage_history, _tick_count=self.eng.tick_count, soul_ref=self.eng.soul, )
...
class PhaseExecutor:
    def execute_phases(self, simulator, ctx):
        active_pipeline = simulator.system_pipeline if getattr(ctx, "is_system_event", False) else simulator.full_pipeline
    ...
    def _audit_flux(ctx, phase, before, after):
        ...
        if abs(b_v - a_v) > 0.01: ctx.record_flux(phase, "voltage", b_v, a_v, "PHASE_DELTA")
        if abs(b_d - a_d) > 0.01: ctx.record_flux(phase, "drag", b_d, a_d, "PHASE_DELTA")
