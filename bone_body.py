    ...
    def _on_neural_shift(self, payload):
        ...
        if state == "PANIC":
            ...
            self.mito.adjust_atp(
                shifts.get("MANIC", {"atp": -10.0}).get("atp", -10.0),
                "Neural Overclock",
            )
...
class MitochondrialForge:
    ...
    def get_status_report(self) -> str:
        ...
        key = "NECROSIS" if atp < 5.0 else "GRINDING" if atp < 20.0 else "APOPTOSIS" if self.state.ros_buildup > 80.0 else "NOMINAL"
    ...
    def process_cycle(
        ...
        if is_critical:
            status = "LOW_POWER"
        atp_collapse = getattr(cfg, "ATP_COLLAPSE", 0.0) if cfg else 0.0
        if self.state.atp_pool <= atp_collapse:
            status = "NECROSIS"
        return MetabolicReceipt(
...
class BioFeedback:
    ...
    def perform_maintenance(self, text: str, phys: Any, logs: List[str], tick: int):
        ...
        if (
            drag > (getattr(cfg, "SLUDGE_DRAG_THRESH", 8.0) if cfg else 8.0)
            and tick % (getattr(cfg, "SLUDGE_TICK_MOD", 10) if cfg else 10) == 0
        ):
            ...
            safe_set(
                ...
                max(
                    1.0,
                    drag - (getattr(cfg, "SLUDGE_DRAG_REDUCTION", 2.0) if cfg else 2.0),
                ),
            )
...
class SomaticLoop:
    ...
    def digest_cycle(
        ...
        max_h = getattr(self.cfg, "MAX_HEALTH", 100.0)
        max_s = getattr(self.cfg, "MAX_STAMINA", 100.0)
        ...
        if delta_silence > 0.6:
            ...
            safe_set(
                phys, "narrative_drag", max(1.0, current_drag - (delta_silence * 2.0))
            )
    ...
    def _package_result(self, resp_status, logs, chem_state=None, enzyme="NONE"):
        return {
            ...
            "enzyme": enzyme,
            "atp": getattr(getattr(self.bio, "mito", None), "state", None).atp_pool if getattr(self.bio, "mito", None) else 60.0,
            "stamina": getattr(self.bio.biometrics, "stamina", 100.0) if getattr(self.bio, "biometrics", None) else 100.0,
        }
...
class MetabolicGovernor:
    ...
    def _get_shift_message(mode: str, text_map: Dict, physics: Any) -> str:
        shift_cfg = (
            LoreManifest.get_instance().get("BODY_CONFIG", "GOVERNOR_SHIFT") or {}
        )
...
class SynestheticCortex:
    ...
    def perceive(
        ...
        counts = safe_get(
            physics,
            "counts",
            safe_get(safe_get(physics, "matter", physics), "counts", {}),
        )
    ...
    def get_current_qualia(
        ...
        tone = (
            strings.get("tones", {}).get(t_key, "")
            if dom_val > 0.2
            else strings.get("tones", {}).get("steady", "")
        )
        hint = (
            strings.get("hints", {}).get(h_key, "")
            if dom_val > 0.05
        ...
        return Qualia(
            color,
            impulse.somatic_reflex or strings.get("reflexes", {}).get("steady", ""),
            tone,
            hint,
        )
    ...
    def apply_impulse(self, impulse: BiologicalImpulse) -> float:
        ...
        for chem in ("cortisol", "oxytocin", "dopamine", "adrenaline"):
            setattr(self.bio.endo, chem, max(0.0, min(1.0, getattr(self.bio.endo, chem) + getattr(impulse, f"{chem}_delta"))))
