...
class MitochondrialForge:
    ...
    def _trigger_anaerobic_bypass(self, raw_cost: float) -> MetabolicReceipt:
        ...
        return MetabolicReceipt(base_cost=raw_cost, drag_tax=0.0, inefficiency_tax=0.0, total_burn=health_burn,
...
class BioFeedback:
    ...
    def perform_maintenance(self, text: str, phys: Any, logs: List[str], tick: int):
        ...
        if drag > safe_get(cfg, "SLUDGE_DRAG_THRESH", 8.0) and tick % safe_get(cfg, "SLUDGE_TICK_MOD", 10) == 0:
...
class SomaticLoop:
    ...
    def digest_cycle(self, text: str, physics_data: Any, fb_dict: Dict, health: float, stamina: float, stress_modifier: float, tick_count: int = 0, circadian_bias: Dict = None, ) -> Dict:
        text = str(text or "")
        phys = physics_data
        logs = []
        ...
        if delta_silence > 0.6:
            ...
            b.stamina = min(safe_get(self.cfg, "MAX_STAMINA", 100.0), b.stamina + (delta_silence * 5.0))
...
class SynestheticCortex:
    ...
    def perceive(self, physics: Dict, traits: Any = None, latency: float = 0.0) -> BiologicalImpulse:
        ...
        counts = safe_get(physics, "counts") or safe_get(safe_get(physics, "matter", {}), "counts", {})
    ...
    def get_current_qualia(
        impulse: Optional[BiologicalImpulse] = None, config_ref=None) -> Qualia:
        ...
        profiles = [(impulse.oxytocin_delta, Prisma.MAG, "resonant", "connect"), (impulse.dopamine_delta, Prisma.CYN, "vibrating", "explore"), (impulse.cortisol_delta, Prisma.OCHRE, "strained", "guarded"), (impulse.adrenaline_delta, Prisma.RED, "urgent", "fast"), ]
