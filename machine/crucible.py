...
class TheCrucible:
    ...
    def dampen(self, voltage_spike: float, stability_index: float) -> Tuple[bool, str, float]:
        ...
        msg_template = ux("physics_strings", "crucible_damper_hit") or "Absorbed -{reduction:.1f} Voltage because {reason}"
    ...
    def audit_fire(self, physics: dict) -> Tuple[str, float, Optional[str]]:
        ...
        if abs(adjustment) > 0.1:
            ...
            direction = ux("machine_strings", "crucible_tightening" if is_tight else "crucible_relaxing") or ("TIGHTENING" if is_tight else "RELAXING")
            msg = ux_format("physics_strings", "crucible_regulator", default="{direction} | Drag: {current:.1f} -> {new:.1f}", direction=direction, current=current_drag, new=final_drag)
        surge = physics.get("system_surge_event", False)
        if surge:
            self.active_state = "SURGE"
            msg = ux_format("physics_strings", "crucible_surge", default="Voltage spike detected ({voltage:.1f}).", voltage=voltage)
        ...
        if voltage > 18.0:
            if structure > 0.5:
                ...
                msg = ux_format("physics_strings", "crucible_ritual", default="High tension converted to capacity. (+{gain:.1f} Cap)", gain=gain)
                return "RITUAL", gain, msg
            else:
                damage = voltage * 0.5
                self.active_state = "MELTDOWN"
                msg = ux_format("physics_strings", "crucible_meltdown", default="Structure failing under voltage. ({damage:.1f} Damage)", damage=damage)
