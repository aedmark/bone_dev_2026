...
class MitochondrialForge:
    ...
    def __init__(self, state_ref: MitochondrialState, events_ref, config_ref=None):
        ...
        # Pull the biological narrative strings (how the system describes its own weariness)
        full_narrative = (LoreManifest.get_instance(config_ref=self.cfg).get("BIO_NARRATIVE") or {})
        self.narrative = full_narrative.get("MITO", {})

        # Absolute limits on how much energy can be expended in a single cycle before damaging the cell.
    ...
    def get_status_report(self) -> str:
        ...
        if atp < 5.0:
            key = "NECROSIS"  # Terminal energy failure.
        elif atp < 20.0:
            key = "GRINDING"  # Dangerously low; cognitive scaffolding is collapsing.
        elif ros > 80.0:
            key = "APOPTOSIS"  # Toxicity is too high; forcing cellular death to save the host.
        else:
            key = "NOMINAL"
    ...
    def adjust_atp(self, delta: float, reason: str = ""):
        ...
        # Only log significant shifts to prevent console bloat.
    ...
    def process_cycle(self, physics_packet: Any, modifier: float = 1.0) -> MetabolicReceipt:
        ...
        """
        # Free pass if the system is perfectly rested.
        ...
        # 1. Base Cost & Geodesic Strain
        # Deeper thoughts and highly connected memories cost more to traverse.
        ...
        # 2. The Chaos Tax
        # If the user's prompt is unstructured or panicked, the engine burns more fuel trying to parse it.
        ...
        # 3. The Amplification Tax (Malignancy & Friction)
        # Penalizes runaway optimization loops. If the system is trying to force an impossible logic structure, friction spikes.
        ...
        # Existing toxicity (ROS) acts as a multiplier on all base demands. Sickness breeds inefficiency.
        ...
        # If starving, the system physically shuts down higher-order processing to survive.
        if is_critical:
        ...
        # Calculate raw cost adjusted by the membrane's health (efficiency).
        ...
        # 4. Waste Generation (ROS Accumulation)
        # Abstract thoughts and chaotic inputs leave behind metabolic byproducts that must eventually be slept off.
        ...
        waste_generated = max(-self.state.ros_buildup, waste_generated)  # Can't have negative total waste

        # Apply the final math to the organism's state.
        self.state.ros_buildup += waste_generated
        self.adjust_atp(-total_metabolic_cost, "Metabolic Burn")

        # Continual high burns permanently degrade the engine's efficiency until it rests.
    ...
    def _apply_adaptive_dynamics(self):
        ...
        if self.state.ros_buildup < ros_sig:
            ...
            # Hormesis zone. The system fights the mild toxicity and increases its own efficiency.
            ...
            # Oxidative stress. Toxicity is overwhelming the engine.
...
class DigestiveTrack:
    ...
    def harvest(self, phys: Any, logs: List[str]) -> Tuple[str, float, int]:
        ...
        # Protect against massive copy-pastes crashing the loop
        ...
        # High voltage prompts (intense logic/creativity) trigger meat digestion for bonus energy.
    ...
    def _digest_words(self, words: List[str]) -> Tuple[float, List[str], float, int]:
        ...
        # Filter out noise words (the, and, it, etc.)
        ...
        for word, count in valid_words.items():
            # Apply the Cliché Tax if the word is a known LLM trope.
            ...
            log_mult = 1.0 + math.log(count)  # Diminishing returns for repeating the same word.
