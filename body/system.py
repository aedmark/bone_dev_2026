...
class BioSystem:
    ...
    def setup_listeners(self):
        ...
        # Distribute narrative strings to the appropriate organs
        if self.mito:
    ...
    def _on_neural_shift(self, payload):
        ...
        if state == "PANIC":
            ...
            # Manic states burn a massive amount of raw ATP instantly
    ...
    def apply_environmental_entropy(self, physics_packet):
        ...
        em_field = math.hypot(ent_val, phi_val) # Electromagnetic stress field
        ...
        # The shield mitigates chaos, but cannot stop thermal buildup completely
...
class SomaticLoop:
    ...
    def digest_cycle(self, text: str, physics_data: Any, fb_dict: Dict, health: float, stamina: float, stress_modifier: float, tick_count: int = 0, circadian_bias: Dict = None) -> Dict:
        ...
        # 1. Clamp Health and Stamina to bounds
        ...
        # 2. Apply passive environmental wear and tear
        self.bio.apply_environmental_entropy(phys)

        # 3. Apply emotional taxes (High Cortisol = higher ATP burn)
        modifier = self.regulator.get_metabolic_modifier(phys, logs)

        # 4. Check for Healing Silence (The act of doing nothing restores Stamina)
        ...
        # 5. Burn ATP based on the physics of the thought
        receipt = self.bio.mito.process_cycle(phys, modifier=modifier)

        # 6. Apply physical damage if we burned anaerobically (panicked/starving)
        ...
        # 7. Check Failsafes (Did the burn kill us?)
        ...
        if safety_status == "MAUSOLEUM_CLAMP":
            ...
            b.stamina = 10.0 # Emergency stamina injection at the cost of health

        # 8. Symbiotic / Digestive Harvest (Gaining energy back)
        total_yield = 0.0
        enzyme = "NONE"
        ...
        # 10. Semantic Assessment (What do the words *mean*?)
        semantic_sig = self.semantic_doctor.assess(clean_words, phys)

        # 11. Somatic Perception (How does the machine *feel* this turn?)
