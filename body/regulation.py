...
class PIDController:
    ...
    def __init__(self, kp, ki, kd, setpoint, output_limits=(-10.0, 10.0)):
        self.kp = kp  # Proportional: Reacts to current error
        self.ki = ki  # Integral: Reacts to accumulation of past errors
        self.kd = kd  # Derivative: Reacts to the rate of change (predicts future error)
        self.setpoint = setpoint # The target ideal state
    ...
    def update(self, measurement: float, dt: float = 1.0, target_override: Optional[float] = None) -> float:
        ...
        # I: Have we been off for a long time?
        if self.ki != 0:
            self._integral += error * safe_dt
            # Anti-windup: Prevent the integral from building up to infinity
            self._integral = max(self.min_out / self.ki, min(self.max_out / self.ki, self._integral))
        else:
            self._integral = 0.0
        I = self.ki * self._integral

        # D: Are we approaching the target too fast? (Apply the brakes)
...
class MetabolicGovernor:
    ...
    hysteresis_duration: int = 3  # Prevents rapid flickering between states
    config_ref: Any = None
    ...
    def __post_init__(self):
        ...
        # Sort thresholds descending by priority/severity
    ...
    def regulate(self, physics: Any, dt: float, endocrine_state: Optional[Any] = None) -> Tuple[float, float]:
        ...
        if endocrine_state:
            ...
            # Cortisol creates a "deadband" — stress makes the system sluggish to respond
            ...
            # Only apply the correction if the error pierces the stress-deadband
    ...
    def shift(self, physics: Dict, _voltage_history: List[float], current_tick: int = 0) -> Optional[str]:
        ...
        # Hysteresis: Don't shift if we just shifted recently (prevents flickering)
...
class BioFeedback:
    ...
    def check_vital_signs(self, phys: Any, stamina: float, logs: List[str]) -> str:
        ...
        if not b:
            ...
            return "MAUSOLEUM_CLAMP"  # Total structural failure
        ...
        if stamina <= 0:
            ...
            # If it's out of health AND energy, execute terminal shutdown
    ...
    def perform_maintenance(self, text: str, phys: Any, logs: List[str], tick: int):
        ...
        # Slowly dissipate 'sludge' (drag) if it's been sitting too long
...
class EndocrineRegulator:
    ...
    def get_metabolic_modifier(self, phys: Any, logs: List[str]) -> float:
        ...
        # High cortisol (stress) increases the cost of thinking (Burnout simulation)
        if chem.cortisol > 0.5:
            stress_tax = 1.0 + (chem.cortisol * 0.5)
            modifier *= stress_tax
            # 30% chance to visibly output a panic string to the log
        ...
        # High adrenaline allows you to think faster/cheaper (Fight or Flight)
        if chem.adrenaline > 0.6:
            modifier *= 0.5
            if msg := ux("endocrine_regulator", "adrenaline_surge"):
                logs.append(f"{Prisma.YEL}{msg}{Prisma.RST}")

        # High dopamine (reward/flow state) makes thinking slightly cheaper
        if chem.dopamine > 0.7:
            modifier *= 0.8

        # High systemic voltage creates friction, making thinking expensive
