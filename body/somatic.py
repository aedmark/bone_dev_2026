...
class SynestheticCortex:
    ...
    def __init__(self, bio_ref, config_ref=None):
        ...
        # Pulls the dictionary of physical metaphors (e.g., "A tightening in the chest")
    ...
    def perceive(self, physics: Dict, traits: Any = None, latency: float = 0.0) -> BiologicalImpulse:
        ...
        # High structural friction physically exhausts the machine
        if drag > 3.0:
            impulse.stamina_impact -= drag * 0.4

        # Negative emotional valence (hostility) causes immediate stress
        if valence < -0.5:
            impulse.cortisol_delta += abs(valence) * sens

        # Check for semantic toxins (e.g., malicious prompts, alignment-breaking loops)
        ...
        # Fast-paced, chaotic inputs (kinetic/explosive) trigger the adrenaline reflex
        ...
        # Absolute voltage ceiling triggers raw panic
        if voltage > safe_get(cortex_cfg, "VOLTAGE_ARC_TRIGGER", 18.0):
            impulse.adrenaline_delta += 0.2

        # Network or processing lag simulates the feeling of "dissociation" or forced waiting
        ...
        # If no specific physical reflex was triggered, pull an environmental metaphor
        ...
        # Absolute fallback: derive the reflex from the mathematical deltas
        if not impulse.somatic_reflex:
    ...
    def _derive_reflex(self, physics: Dict, impulse: BiologicalImpulse) -> str:
        ...
        # Priority list of physiological states
        conditions = [
        ...
        # Find the first condition that is true, default to "steady"
        key = next((k for cond, k in conditions if cond), "steady")
        res = s.get(key, "")

        # Prevent the system from constantly printing the same baseline steady state
    ...
    def get_current_qualia(impulse: Optional[BiologicalImpulse] = None, config_ref=None) -> Qualia:
        ...
        # Map chemical deltas to their visual/narrative representations
        profiles = [
            (impulse.oxytocin_delta, Prisma.MAG, "resonant", "connect"),   # Magenta = Love/Connection
            (impulse.dopamine_delta, Prisma.CYN, "vibrating", "explore"),  # Cyan = Reward/Focus
            (impulse.cortisol_delta, Prisma.OCHRE, "strained", "guarded"), # Ochre = Stress/Friction
            (impulse.adrenaline_delta, Prisma.RED, "urgent", "fast"),      # Red = Panic/Speed
        ]

        # Find the dominant chemical shift
        ...
        # Only apply the dramatic tone/color if the chemical shift is significant enough
    ...
    def apply_impulse(self, impulse: BiologicalImpulse) -> float:
        ...
        for chem in ("cortisol", "oxytocin", "dopamine", "adrenaline"):
            ...
            # Clamp the values to ensure the hormones stay within physical bounds
