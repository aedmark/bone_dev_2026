...
class TestDrivers(BoneTestCase):
    def setUp(self):
        super().setUp()
        self.chaotic_config = {
            "DRIVERS": {"VSL_E_GROWTH_MULT": "0.1", "VSL_FATIGUE_MULT": "0.5", "VSL_LIMINAL_THRESHOLD": "0.5",
                        "ENNEAGRAM_HYSTERESIS": "2", "PROFILE_MIN_WORDS": "2", "PROFILE_ALPHA_HIGH": "0.5",
                        "LIMINAL_LEXICAL_WEIGHT": "0.5", "SYNTAX_STRESS_PUNCTUATION": "0.1",
        ...
        self.mock_manifest_data = {
            ...
            "LENSES": {
                "OBSERVER": {"vocab": "watch,see", "keywords": ["witness"]}
            }
        }
        ...
        self.mock_lexicon.get_categories_for_word.side_effect = lambda w: ["heavy"] if w == "iron" else ["void"] if w == "abyss" else []
    ...
    def test_consultant_type_coercion_and_growth(self):
        consultant = BoneConsultant(config_ref=self.chaotic_config, lexicon_ref=self.mock_lexicon)
        loose_physics = {"beta_index": "0.8", "narrative_drag": "2.0"}
        consultant.update_coordinates("Just a few words.", bio_state={"fatigue": 0.5}, physics=loose_physics)
        self.assertTrue(isinstance(consultant.state.E, float), "[FAIL] State E failed to cast to float.")
        self.assertTrue(isinstance(consultant.state.B, float), "[FAIL] State B failed to cast to float.")
        self.assertGreater(consultant.state.E, 0.1, "[FAIL] E growth multiplier failed to execute.")
    ...
    def test_consultant_system_prompt_routing(self):
        ...
        self.assertIn("Embrace the void.", prompt, "[FAIL] Consultant failed to route to Liminal directive.")

    def test_enneagram_raw_persona_loose_physics(self):
        enneagram = EnneagramDriver(events_ref=MagicMock(), config_ref=self.chaotic_config)
        chaotic_physics = {"voltage": "15.0", "narrative_drag": "5.0", "kappa": "0.2", "vector": {"ENT": "0.9"}}
        persona, state, reason = enneagram._calculate_raw_persona(chaotic_physics)
        self.assertIsNotNone(persona, "[FAIL] Enneagram crashed while attempting to coerce dictionary physics.")
        self.assertIn("THE MANIC", persona, "[FAIL] Persona failed to evaluate high-tension string bounds.")
    ...
    def test_enneagram_hysteresis_lock(self):
        ...
        self.assertEqual(p1, "NARRATOR", "[FAIL] Enneagram shifted immediately, ignoring hysteresis.")
        p2, _, _ = enneagram.decide_persona(shifting_physics)
        self.assertNotEqual(p2, "NARRATOR", "[FAIL] Enneagram failed to shift after surpassing hysteresis threshold.")

    def test_liminal_dark_matter_sparks(self):
        liminal = LiminalModule(config_ref=self.chaotic_config, lexicon_ref=self.mock_lexicon)
        text = "iron abyss iron abyss"
        loose_vector = {"PSI": "0.8", "ENT": "0.5"} # String vector
        score = liminal.analyze(text, loose_vector)
        self.assertTrue(isinstance(score, float), "[FAIL] Liminal analysis failed to return a float.")
        self.assertGreater(score, 0.1, "[FAIL] Failed to calculate dark matter lambda from alternating lexical categories.")
