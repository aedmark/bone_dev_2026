...
class TestLexicon(BoneTestCase):
    ...
    def test_store_harvest_punctuation_stripping(self):
        ...
        self.assertIn(
            "monolith",
            harvested["heavy"],
            "[FAIL] Failed to strip punctuation from 'monolith,'.",
        )
        self.assertIn(
            "sprint",
            harvested.get("kinetic", []),
            "[FAIL] Failed to strip punctuation and match exactly.",
        )
    ...
    def test_store_harvest_type_coercion(self):
        ...
        self.assertEqual(
            result,
            {},
            "[FAIL] Harvest crashed or returned invalid data when fed an integer.",
        )
    ...
    def test_store_teach_limit_enforcement(self):
        ...
        self.assertEqual(
            len(learned),
            1000,
            "[FAIL] LexiconStore failed to prune oldest words when limit was reached.",
        )
        self.assertNotIn(
            "word_0",
            learned,
            "[FAIL] LexiconStore pruned the wrong word. The oldest word survived.",
        )
    ...
    def test_analyzer_sanitize_type_coercion(self):
        ...
        self.assertTrue(
            ...
            "[FAIL] Sanitize failed to return a list for a namespace object.",
        )
        ...
        self.assertIn(
            "12345",
            result_num,
            "[FAIL] Sanitize failed to coerce and parse an integer.",
        )
    ...
    def test_analyzer_antigen_purge(self):
        ...
        self.assertNotIn(
            "basically", clean_text, "[FAIL] Antigen 'basically' survived sanitization."
        )
        self.assertIn(
            "figuratively",
            clean_text,
            "[FAIL] Antigen 'literally' was not replaced with 'figuratively'.",
        )
    ...
    def test_analyzer_valence_negation(self):
        ...
        self.assertGreater(
            self.analyzer.measure_valence(pos_words),
            0.0,
            "[FAIL] Positive words scored <= 0.",
        )
        self.assertLess(
            self.analyzer.measure_valence(neg_words),
            0.0,
            "[FAIL] Negative words scored >= 0.",
        )
        negated_score = self.analyzer.measure_valence(negated_words)
        self.assertLess(
            negated_score,
            0.0,
            "[FAIL] The negator 'not' failed to invert the positive valence of 'perfect'.",
        )
    ...
    def test_analyzer_turbulence_bounds(self):
        ...
        self.assertEqual(
            turb_single,
            0.0,
            "[FAIL] Turbulence for a single word should be exactly 0.0.",
        )
        turb_wild = self.analyzer.get_turbulence(
            ["a", "supercalifragilisticexpialidocious", "is", "wild"]
        )
        self.assertGreater(
            turb_wild,
            0.5,
            "[FAIL] High variance in word lengths failed to generate significant turbulence.",
        )
    ...
    def test_service_event_bus_loose_payload(self):
        ...
        self.assertIn(
            "heavy",
            service.get_categories_for_word("gestalt"),
            "[FAIL] Service failed to learn from dict payload.",
        )
        ns_payload = SimpleNamespace(word="synapse", category="kinetic")
        service._on_mythology_update(ns_payload)
        self.assertIn(
            "kinetic",
            service.get_categories_for_word("synapse"),
            "[FAIL] Service failed to learn from Namespace payload.",
        )
    ...
    def test_semantic_field_momentum(self):
        ...
        self.assertIn(
            "VEL", vec1, "[FAIL] Field failed to map 'kinetic' to 'VEL' dimension."
        )
        field.update("monolith iron gravity")
        self.assertGreater(
            field.momentum,
            0.0,
            "[FAIL] Massive semantic shift failed to generate momentum/flux.",
        )
