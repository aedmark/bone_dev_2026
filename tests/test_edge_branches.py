...
class EdgeBranchCoverageTests(BoneTestCase):
    ...
    def test_congruence_validator_bonus_branch(self, MockLore):
        ...
        mock_lore_instance.get.return_value = {
            "OBSERVER": {"vocab": "tensegrity, structure", "keywords": ["manifold"]}
        }
        ...
        score = validator.calculate_resonance(
            "tensegrity manifold noise", MockContext()
        )
        self.assertGreater(
            score, 0.8, "Validator failed to execute the resonance bonus branch."
        )
    ...
    def test_therapy_healing_threshold_branch(self):
        ...
        phys_fixture = {"matter": {"counts": {"toxin": 0}, "vector": {"STR": 0.5}}}
        trauma_accum = {"SEPTIC": 10.0}
        healed = therapy.check_progress(phys_fixture, 100.0, trauma_accum)
        self.assertEqual(
            len(healed), 0, "Therapy executed the healing branch too early."
        )
        healed = therapy.check_progress(phys_fixture, 100.0, trauma_accum)
        self.assertIn("SEPTIC", healed, "Therapy failed to execute the healing branch.")
        self.assertLess(
            trauma_accum["SEPTIC"], 10.0, "Trauma was not successfully reduced."
        )
