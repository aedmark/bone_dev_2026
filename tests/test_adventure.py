...
class AppendLogger:
    ...
    def log(self, test_id, category, *args, **kwargs):
        with open(self.filename, 'a', encoding='utf-8') as f:
            ...
            if args: f.write(f"Args: {args}\n")
            if kwargs: f.write(f"Kwargs: {json.dumps(kwargs, default=str)}\n")
...
class BoneTestCase(unittest.TestCase):
    def setUp(self):
        ...
        # FIX: The lambda evaluated to None, crashing dependent asserts.
        # We now log the event and return a string representing the output filepath.
        def _hooked_chronos_save(*args, **kwargs):
            self.lore_logger.log(self.id(), "CHRONOS QUICKSAVE", args=args, kwargs=kwargs)
        ...
        try:
            ...
            print(f"{Prisma.RED}Test Engine Initialization Failed! Captured Output is preserved in test_output_full.log{Prisma.RST}")
            raise
