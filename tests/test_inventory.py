...
class InventorySystemTests(BoneTestCase):
    ...
    def test_container_pack_and_unpack(self):
        ...
        # 1. Setup our items
        gordon.register_dynamic_item("LEATHER_POUCH", {
        ...
        self.assertNotIn("MAP", gordon.inventory, "MAP should be removed from main inventory.")
        ...
        self.assertIn("MAP", gordon.inventory, "MAP should be returned to main inventory.")
    ...
    def test_container_capacity_and_nesting_limits(self):
        ...
        # Setup items
        gordon.register_dynamic_item("BOX", {
            "is_container": True,
            "capacity": 1,
            "contents": []
        })
        ...
        # 2. Test Infinite Recursion (Putting an item inside itself)
        ...
        # 3. Test Non-Containers
        # The ROCK is in the box, but we still hold the STONE and the BOX.
        success4, msg4 = gordon.pack_item("BOX", "STONE")
        self.assertFalse(success4, "Gordon allowed packing into an item that isn't a container.")
    ...
    def test_container_parser_regex(self):
        ...
        # Setup items
        gordon.register_dynamic_item("MAGIC_BAG", {
            "is_container": True,
            "capacity": 5,
            "contents": []
        })
        ...
        # 1. Test the "Put" Regex
        logs, handled = gordon.process_container_commands("I quickly put the rusty sword in my magic bag.")
        self.assertTrue(handled, "Regex failed to capture the 'put in' phrase.")
        self.assertNotIn("RUSTY_SWORD", gordon.inventory, "Sword was not removed from belt.")

        bag = gordon.get_item_data("MAGIC_BAG")
        self.assertIn("RUSTY_SWORD", bag.contents, "Sword was not added to the bag.")

        # 2. Test the "Take" Regex
        logs, handled = gordon.process_container_commands("take the rusty sword out of the magic bag please")
        self.assertTrue(handled, "Regex failed to capture the 'take from' phrase.")
        self.assertIn("RUSTY_SWORD", gordon.inventory, "Sword was not returned to belt.")
