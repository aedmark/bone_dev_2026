...
class InventorySystemTests(BoneTestCase):
    def test_pinker_substring_contamination(self):
        ...
        self.assertNotIn("GOLD_COIN", gordon.inventory, "Gordon incorrectly auto-looted due to sub-string contamination ('forget' -> 'get').")
    ...
    def test_container_parser_regex(self):
        engine = BoneAmanita({})
        gordon = getattr(engine.village, "gordon", None)
        gordon.register_dynamic_item("MAGIC_BAG", {"is_container": True, "capacity": 5, "contents": []})
