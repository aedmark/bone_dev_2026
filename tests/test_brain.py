...
class BrainSubstrateTests(BoneTestCase):
    ...
    def test_dream_engine_terminal_sleep_failure(self, mock_random):
        ...
        self.assertEqual(
            shift.get("voltage"),
            100.0,
            "[FAIL] Terminal sleep did not instantly spike the voltage.",
        )
    ...
    def test_cerebral_tunnel_vision(self):
        ...
        res_stress = idx.query_neighborhood(
            [0.1, 0.1], k=3, physics_state={"cortisol": 0.9}
        )
        ids = [r["id"] for r in res_stress]
        self.assertNotIn(
            "playful",
            ids,
            "[FAIL] Cortisol failed to block positive dimensions (Tunnel Vision failed).",
        )
        self.assertLess(
            len(res_stress), 3, "[FAIL] Cortisol failed to clamp the FAISS K limit."
        )
