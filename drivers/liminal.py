...
class LiminalModule:
    ...
    def analyze(self, text: str, physics_vector: Dict[str, float]) -> float:
        ...
        lexical_lambda = min(1.0, void_hits * float(safe_get(cfg, "LIMINAL_LEXICAL_WEIGHT", 0.15)))
        ...
        dark_matter_lambda = min(
            1.0, dark_matter_sparks * float(safe_get(cfg, "LIMINAL_DARK_MATTER_WEIGHT", 0.25)))
        pv = physics_vector or {}
        vector_lambda = ((float(pv.get("PSI", 0)) * float(safe_get(cfg, "LIMINAL_VEC_PSI_MULT", 0.5))) +
                         (float(pv.get("ENT", 0)) * float(safe_get(cfg, "LIMINAL_VEC_ENT_MULT", 0.3))) +
                         (float(pv.get("DEL", 0)) * float(safe_get(cfg, "LIMINAL_VEC_DEL_MULT", 0.2))))
