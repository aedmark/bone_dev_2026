...
class LiminalModule:
    ...
    def analyze(self, text: str, physics_vector: Dict[str, float], grammatical_stress: float = 0.0, momentum: float = 0.0) -> float:
        w = self.weights
        ...
        if is_psychoanalyzing:
            ...
            self.active_trauma = max(0.0, self.active_trauma - (momentum * w["trauma_heal"]))
    ...
    def test_liminal_trauma_aggravation(self):
        ...
        liminal.analyze("iron abyss", loose_vector, grammatical_stress=0.8, momentum=10.0)
        self.assertGreater(liminal.active_trauma, 2.0, "[FAIL] Trauma failed to aggravate under high momentum and drag.")
        self.assertEqual(liminal.godel_scars, 0, "[FAIL] Scars mutated when only the active wound should have spiked.")
    ...
    def test_liminal_trauma_healing(self):
        ...
        liminal.analyze("iron abyss", loose_vector, grammatical_stress=0.1, momentum=10.0)
        self.assertLess(liminal.active_trauma, 2.0, "[FAIL] Active trauma failed to heal/decay under a high momentum flow state.")
        self.assertEqual(liminal.godel_scars, 0, "[FAIL] Scars mutated during a healing cycle.")
    ...
    def test_liminal_scar_permanence(self):
        ...
        self.assertEqual(liminal.godel_scars, 1, "[FAIL] Godel scar failed to record the permanent mathematical trauma.")
        self.assertEqual(liminal.active_trauma, 1.0, "[FAIL] Active symptom (wound) failed to spike alongside scar creation.")
        self.assertLess(liminal.lambda_val, 0.8, "[FAIL] Lambda failed to apply its relief modifier after triggering a scar.")